...
「Messaging API 設定」タブにある「応答メッセージ」「あいさつメッセージ」の
「編集」ボタンは、クリックすると LINE Official Account Manager に遷移します。
LINE Developers Console 単体では応答系の設定は変更できない、と覚えておいてください。

##### LINE Developers Console 側の設定
...
##### LINE Official Account Manager 側の設定

[https://manager.line.biz/](https://manager.line.biz/) にログインし、
今作ったアカウントを選択 → 右上の「設定」→ 左メニューの「応答設定」を開き、
以下のように設定してください:
...
> [!NOTE]
> 「応答モード」が「チャット」のままだと、Webhook が送られても bot が反応しません。
> 必ず「Bot」に切り替えてください。
> 2022 年 11 月以降は Webhook とチャットの併用も可能ですが、
> 今日はシンプルに「Bot」モードで進めます。
